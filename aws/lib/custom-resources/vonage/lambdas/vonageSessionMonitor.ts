...
async function putCallbacks(event: CloudFormationCustomResourceCreateEvent | CloudFormationCustomResourceUpdateEvent) {
    ...
    async function createCallback(group: "connection" | "stream", event: "created" | "destroyed") {
    ...
    try {
        const callback1 = await createCallback("connection", "created");
        const callback2 = await createCallback("connection", "destroyed");
        const callback3 = await createCallback("stream", "created");
        const callback4 = await createCallback("stream", "destroyed");

        return {
            PhysicalResourceId: `${callback1}-${callback2}-${callback3}-${callback4}`,
            Data: {
                Callback1Id: callback1,
                Callback2Id: callback2,
                Callback3Id: callback3,
                Callback4Id: callback4,
            },
        };
    } catch (e: any) {
        if (e.toString().includes("status code 400")) {
            throw new Error(
                "Unable to configure Vonage session monitoring callback. Status code 400. This usually means that Vonage tried to ping the callback url and found it couldn't reach it. It is sufficient to just run PacketRiot for the relevant domain."
            );
        }
        throw e;
    }
