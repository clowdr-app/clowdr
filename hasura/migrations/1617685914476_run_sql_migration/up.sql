...
CREATE TRIGGER duplication_insert AFTER INSERT 
    ON "chat"."Message" 
    FOR EACH ROW 
    WHEN (pg_trigger_depth() < 1 AND NEW."duplicatedMessageId" IS NULL AND NEW."duplicatedMessageSId" IS NULL)
...
CREATE TRIGGER duplication_insert AFTER INSERT 
    ON "chat"."Reaction" 
    FOR EACH ROW 
    WHEN (pg_trigger_depth() < 1 AND NEW."duplicateId" IS NULL AND NEW."duplicateSId" IS NULL)
...
BEGIN
    UPDATE "chat"."Message"
        ...
        WHERE "chat"."Message"."id" = NEW."duplicatedMessageId"
		   OR "chat"."Message"."sId" = NEW."duplicatedMessageSId";
	RETURN NEW;
END;
$$;
...
BEGIN
    UPDATE "chat"."Reaction"
        ...
        WHERE "chat"."Reaction"."id" = NEW."duplicateId"
		   OR "chat"."Reaction"."sId" = NEW."duplicateSId";
	RETURN NEW;
END;
$$;
...
CREATE TRIGGER duplication_update AFTER UPDATE 
    ON "chat"."Message" 
    FOR EACH ROW 
    WHEN (pg_trigger_depth() < 1 AND NEW."duplicatedMessageId" IS NOT NULL AND NEW."duplicatedMessageSId" IS NOT NULL)
...
CREATE TRIGGER duplication_update AFTER UPDATE 
    ON "chat"."Reaction" 
    FOR EACH ROW 
    WHEN (pg_trigger_depth() < 1 AND NEW."duplicateId" IS NOT NULL AND NEW."duplicateSId" IS NOT NULL)
