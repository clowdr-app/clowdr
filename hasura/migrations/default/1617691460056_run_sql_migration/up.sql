...
CREATE TRIGGER duplication_insert AFTER INSERT 
    ON "chat"."Message" 
    FOR EACH ROW 
    WHEN (pg_trigger_depth() < 1 AND (NEW."duplicatedMessageId" IS NULL OR NEW."duplicatedMessageSId" IS NULL))
...
CREATE TRIGGER duplication_insert AFTER INSERT 
    ON "chat"."Reaction" 
    FOR EACH ROW 
    WHEN (pg_trigger_depth() < 1 AND (NEW."duplicateId" IS NULL OR NEW."duplicateSId" IS NULL))
...
CREATE TRIGGER duplication_update AFTER UPDATE 
    ON "chat"."Message" 
    FOR EACH ROW 
    WHEN (pg_trigger_depth() < 1 AND (NEW."duplicatedMessageId" IS NOT NULL OR NEW."duplicatedMessageSId" IS NOT NULL))
...
CREATE TRIGGER duplication_update AFTER UPDATE 
    ON "chat"."Reaction" 
    FOR EACH ROW 
    WHEN (pg_trigger_depth() < 1 AND (NEW."duplicateId" IS NOT NULL OR NEW."duplicateSId" IS NOT NULL))
