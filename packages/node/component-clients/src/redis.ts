...
export const redisClientP = {
    ...
    hdel: (redisClient: RedisClient) =>
        promisify((key: string, field: string, cb?: Callback<number>) => redisClient.hdel(key, field, cb)),
    hgetall: (redisClient: RedisClient) =>
        promisify((key: string, cb?: Callback<{ [key: string]: string }>) => redisClient.hgetall(key, cb)),
    hmset: (redisClient: RedisClient) =>
        promisify((key: string, value: { [key: string]: string }, cb?: Callback<string>) =>
            redisClient.hmset(key, value, cb)
        ),
